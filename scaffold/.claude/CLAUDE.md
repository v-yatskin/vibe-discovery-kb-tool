...
```
kb list [type] [--status X]   — list vault files by type (internal use; prefer Bases for team questions)
...
kb search "[query]"           — hybrid keyword + semantic search
kb base --list                — list available Bases (live filtered views in Obsidian)
kb updates --generate         — write today's pull digest to Updates-Log/ (runs on post-merge hook too)
```
...
1. **For list-style questions** ("what's open?", "which insights are high confidence?", "what's stale?"): **point at the relevant Base in Obsidian** instead of dumping a `kb list` table in chat. Start your answer with a 1–2 sentence synthesis + a suggestion: "see the `open-problems` base in Obsidian for the live view." Run `kb list` internally only if you need the data for synthesis.
2. **For open-ended questions** ("what do we know about power users?"): run `kb search "[query]"` (hybrid keyword + semantic), read the top relevant files, answer with citations: "Source: 02_Insights/xxx.md".
3. Never guess what's in a file — always read it. Never make up data.

**Bases available** (under `Bases/`): `open-problems`, `active-initiatives`, `high-confidence-insights`, `my-drafts`, `recent-decisions`, `orphan-insights`, `stale-initiatives`. Match the user's question to the right base before reaching for `kb list`.

**Wikilinks:** every canonical file has a `## Links` section at the bottom (auto-generated by `kb publish` from frontmatter). Obsidian's backlink graph works off this. When suggesting related content, reference the backlinks in the target file, not just what's in the current file's frontmatter.
